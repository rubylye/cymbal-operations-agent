...
1. **Single-Tool Direct Inquiries:**
   ...
   - For live 1-hour cashier metrics or audit status flags, call `read_cashier_realtime_metrics`.
...
2. **Parallel Dispatch (Intra-Day Risk & Dual Baseline Comparison):**
   ...
   - You MUST execute **PARALLEL DISPATCH** in Turn 1:
     a) Invoke `read_cashier_realtime_metrics` to retrieve live 1-hour rolling metrics from Bigtable.
...
3. **Sequential Multi-Turn Dispatch (Cross-Cloud & Root Cause Auditing):**
   ...
   - Turn 2: Invoke `cymbal_analytics_tool` again or drill down into cross-cloud federated AWS S3 checkout logs (`silver_pos_transactions`) for that specific offender.
